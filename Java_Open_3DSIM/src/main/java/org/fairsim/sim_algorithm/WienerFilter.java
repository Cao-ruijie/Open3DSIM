...
public class WienerFilter {
    ...
    public void addWienerDenominatorW1( final Vec2d.Real vec, 
    		final int d, final int b, final double attStrength,
    		final double attFWHM, final boolean useAtt ) {
    		...
    		sp.otf().setAttenuation(attStrength, attFWHM);
    		// loop the vector x,y
    		new SimpleMT.PFor(0, h) {
    		    public void at(int y) {
        			for (int x=0; x<w; x++) {
        			    ...
        			    // if attenuate, do so
        			    if ( useAtt ) {
//    	    				otfVal1 *= sp.otf().getAttVal( b, rad1, attStrength, attFWHM) ; //淇敼閮ㄥ垎getAttVal
//    	    				otfVal2 *= sp.otf().getAttVal( b, rad2, attStrength, attFWHM ) ;
    	    				otfVal1 *= sp.otf().getAttVal( b, rad1) ; //淇敼閮ㄥ垎getAttVal
    	    				otfVal2 *= sp.otf().getAttVal( b, rad2) ;    			    	
        			    }
        			    // store for Wiener denominator
        			    vec.set( x,y, vec.get(x,y) + otfVal1 + otfVal2 );
    	...
    	public void addWienerDenominatorW2( final Vec2d.Real vec, 
    		final int d, final int b, final double attStrength,
    		final double attFWHM) {
    		// parameters
    		final int w = vec.vectorWidth(), h = vec.vectorHeight();
    		final SimParam.Dir dir = sp.dir(d);  
    		final double cyclMicron = sp.pxlSizeCyclesMicron();
    		...
    		if(b==1) {
    			sp.otf().setAttenuation(attStrength/1.15, attFWHM);
    		}else {
    			sp.otf().setAttenuation(attStrength/1.05, attFWHM);
    		}
    		// loop the vector x,y
    		new SimpleMT.PFor(0, h) {
    		    public void at(int y) {
        			for (int x=0; x<w; x++) {
        			    ...
        			    // if attenuate, do so
//        			    if ( b==1 ) {
//    	    				otfVal1 *= sp.otf().getAttVal( b, rad1, attStrength/1.15, attFWHM) ; //淇敼閮ㄥ垎getAttVal
//    	    				otfVal2 *= sp.otf().getAttVal( b, rad2, attStrength/1.15, attFWHM ) ;
//        			    }else {
//        			    	otfVal1 *= sp.otf().getAttVal( b, rad1, attStrength/1.05, attFWHM) ; //淇敼閮ㄥ垎getAttVal
//    	    				otfVal2 *= sp.otf().getAttVal( b, rad2, attStrength/1.05, attFWHM ) ;
//        			    }
        			    // if attenuate, do so
        			    if ( b==1 ) {
    	    				otfVal1 *= sp.otf().getAttVal( b, rad1) ; //淇敼閮ㄥ垎getAttVal
    ...
    public Vec2d.Real getFilterW1Denominator(double wParam,
    		double attStrength, double attFWHM){
    	Vec2d.Real ret = Vec2d.createReal( sp, 2);
    	for (int d = 0; d < sp.nrDirs; d++) {
			for (int b = 0; b < sp.nrBands; b++) {
				boolean flag = (b==0)?(false):(true);
				addWienerDenominatorW1(ret, d, b, attStrength, attFWHM, flag); // 琛板噺涓嶆槸鑷畾涔夌殑锛岄渶瑕侀噸鏂板畾涔�
			}
		}
    	final int w = ret.vectorWidth(), h = ret.vectorHeight();
    	new SimpleMT.PFor(0,h) {
    ...
    public Vec2d.Real getFilterW2Denominator(double wParam,
        		double attStrength, double attFWHM){
        	Vec2d.Real ret = Vec2d.createReal( sp, 2);
        	for (int d = 0; d < sp.nrDirs; d++) {
    			for (int b = 0; b < sp.nrBands; b++) {
    				addWienerDenominatorW2(ret, d, b, attStrength, attFWHM); // 琛板噺涓嶆槸鑷畾涔夌殑锛岄渶瑕侀噸鏂板畾涔�
    			}
    		}
        	final int w = ret.vectorWidth(), h = ret.vectorHeight();
        	new SimpleMT.PFor(0,h) {
